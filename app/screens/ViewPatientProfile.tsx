...
const ViewPatientProfile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchClinicalData = async () => {
      try {
        const response = await axios.get(
          `https://v6fdr37z-8000.inc1.devtunnels.ms/api/api/clinical-data/?patient_id=${patientID}`
        );
    ...
    const fetchMetabolicData = async () => {
      try {
        const response = await axios.get(
          `https://v6fdr37z-8000.inc1.devtunnels.ms/api/api/metabolic-data/?patient_id=${patientID}`
        );
